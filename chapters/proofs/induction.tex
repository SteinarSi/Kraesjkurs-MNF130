...
\begin{frame}{Matematisk induksjon}
    \begin{columns}
        \begin{column}{0.65\textwidth}
            ...
            \pause
            Siden $P(0)$ stemmer, må $P(1)$ stemme óg\\
            Siden $P(1)$ stemmer, må $P(2)$ stemme óg\\
            Siden $P(2)$ stemmer, må $P(3)$ stemme óg\\
            \ldots\\
...
\begin{frame}{Oppgave: vis at $\sum_{i=0}^{n} i = 0 + 1 + 2 + \ldots + n = \frac{n(n+1)}{2}$.}
    \pause    
    ...
    \pause
    $\sum_{i=0}^{k+1} i = 0 + 1 + 2 + \ldots + k + (k+1)$ \pause $= (\sum_{i=0}^{k} i )+ (k+1)$\\
    \pause
    $=\frac{k(k+1)}{2} + k + 1$\\
    \pause
...
\begin{frame}{Sterk induksjon}
    Dette er veldig likt. Men istedet for å anta $P(k)$, antar man $P(0) \land P(1) \land \ldots \land P(k)$.\\
...
\begin{frame}{Rekursive strukturer}
    ...
    \begin{block}{Eksempel}
        ...
        $\mathbb{S} = \{7, 70, 700, 7000, 70000, \ldots\}$
    \end{block}
\end{frame}
...
% Jeg tror dette er for skremmende
