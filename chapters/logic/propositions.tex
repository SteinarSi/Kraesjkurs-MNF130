...
\begin{frame}{Proposisjoner}
    ...
    \begin{block}{Proposisjoner}
        $p :\iff 2 < 3$ \\
        $q :\iff$ ``Månen er laget av ost''\\
        $r :\iff $``MNF130 er et nyttig fag for informatikere''
    \end{block}
    \pause
...
\begin{frame}{$\lnot$ og sannhetstabeller}
    ...
    \begin{block}{Eksempler}
        La $p :\iff 2 < 3$. \\
        $p \iff$ ``Det er sant at 2 er mindre enn 3'' \\
        $\lnot p \iff$ ``Det er ikke sant at 2 er mindre enn 3''
    \end{block}
    \pause
    \begin{center}
        \begin{tabular}{c|c}
...
\begin{frame}{$\lor$ og $\land$}
    ...
    \begin{block}{Eksempel}
        La $p : \iff$ ``Jorden er flat'', og $q : \iff$ ``Månen er flat''\\
        $p \lor q \iff$ ``Jorden er flat \textbf{eller} månen er flat''\\
        $p \land q \iff$ ``Jorden er flat \textbf{og} månen er flat''\\
    \end{block}
    \pause
    \begin{center}
...
\begin{frame}{$\rightarrow$ og $\leftrightarrow$}
    ...
    \end{itemize}
    \begin{block}{Eksempel}
        La $p : \iff$ ``Det regner ute'', og $q : \iff$ ``Bakken er våt''\\
        $p \rightarrow q \iff$ ``Hvis det regner ute, blir bakken våt''\\
        $p \leftrightarrow q \iff$ ``Det regner ute hvis og bare hvis bakken er våt''
    \end{block}
    \pause
    \begin{center}
        \begin{tabular}{c|c|c|c}
...
\begin{frame}{Konvers, kontrapositiv, og invers}
    \begin{itemize}
        \item Vi kan lage tre nye implikasjoner fra en implikasjon $p \rightarrow q$\\
    \end{itemize}
    \pause
    \begin{center}
        \begin{tabular}{c|c}
            Navn & Implikasjon \\ \hline
    ...
    \begin{block}{Eksempel}
        La $p : \iff$''Det regner ute'', og $q : \iff$ ''Bakken er våt''\\
        $p \rightarrow q \iff$ ''Hvis det regner ute, blir bakken våt''\\
        $q \rightarrow p \iff$ ''Hvis bakken er våt, så regner det ute''\\
        $\lnot q \rightarrow \lnot p \iff$ ''Hvis bakken ikke er våt, så regner det ikke ute''\\
        $\lnot p \rightarrow \lnot q \iff$ ''Hvis det ikke regner ute, blir bakken ikke våt''\\
    \end{block}
\end{frame}
...
\begin{frame}{Egenskaper}
    ...
    \begin{itemize}
        \item To uttrykk har like kolonner $\leftrightarrow$ uttrykkene er logisk ekvivalente
        \pause
        \item Et uttrykk har kun T-er $\leftrightarrow$ uttrykket er en tautologi
        \pause
        \item Et uttrykk har minst én T $\leftrightarrow$ uttrykket er tilfredsstilbart
    \end{itemize}
\end{frame}
...
\begin{frame}{Predikater}
    ...
    \begin{block}{Eksempler på predikater}
        $P(x, y, z) : \iff x + y < z$\\
        $Q(s) : \iff $ ``The string $s$ contains the letter \enquote{a}''
    \end{block}
    \pause
    \begin{block}{Eksempler på evaluering}
        $P(1, 2, 3) \iff 1 + 2 < 3 \iff 3 < 3 \equiv F$ \\ 
        $Q($\enquote{Carina}$) \iff $ ``The string \enquote{Carina} contains the letter \enquote{a}''$ \equiv T$
    \end{block}
