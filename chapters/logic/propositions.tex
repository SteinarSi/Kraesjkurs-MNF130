...
\begin{frame}{$\lnot$ og sannhetstabeller}
    \begin{itemize}
        \item Logiske \emph{operatorer} lar oss ``regne'' med preposisjoner.
        \item Gitt en proposisjon $p$, representerer vi den motsatte proposisjonen som $\lnot p$.
...
\begin{frame}{$\lor$ og $\land$}
    \begin{itemize}
        \item Vi kan slå sammen flere proposisjoner til en \emph{sammensatt} proposisjon med forskjellige operatorer.
    \end{itemize}
...
\begin{frame}{$\rightarrow$ og $\leftrightarrow$}
    ...
    \begin{block}{Eksempel}
        ...
        $p \rightarrow q = $ ``Hvis det regner ute, så er bakken våt''\\
...
\begin{frame}{Konvers, kontrapositiv, og invers}
    ...
    \begin{block}{Eksempel}
        La $p : = $''Det regner ute'', og $q : = $ ''Bakken er våt''\\
        $p \rightarrow q = $ ''Hvis det regner ute, så er bakken våt''\\
        $q \rightarrow p = $ ''Hvis bakken er våt, så regner det ute''\\
        $\lnot q \rightarrow \lnot p = $ ''Hvis bakken ikke er våt, så regner det ikke ute''\\
        $\lnot p \rightarrow \lnot q = $ ''Hvis det ikke regner ute, blir bakken ikke våt''\\
    \end{block}
\end{frame}
...
\begin{frame}{Egenskaper}
    \begin{itemize}
        ...
        \pause
        \item Om et uttrykk alltid er usant, uavhengig av innholdet, kaller vi det en \emph{kontradiksjon}.
    ...
    \begin{itemize}
        ...
        \pause
        \item Konklusjon 3: $p \land \lnot p \equiv F$, og er en kontradiksjon.
    \end{itemize}
    \pause
    ...
    \begin{itemize}
        ...
        \pause
        \item Et uttrykk har kun F-er $\leftrightarrow$ uttrykket er en kontradiksjon
    \end{itemize}
\end{frame}
...
\begin{frame}{Predikater}
    ...
    \begin{block}{Eksempler på predikater}
        $P(x, y, z) : =  x + y < z$\\
        $Q(s) : = $ ``The string $s$ contains the letter \enquote{a}''
    \end{block}
    \pause
    \begin{block}{Eksempler på evaluering}
        $P(1, 2, 3) \equiv 1 + 2 < 3 \equiv 3 < 3 \equiv F$ \\ 
        $Q($\enquote{Olav}$) \equiv $ ``The string \enquote{Olav} contains the letter \enquote{a}''$ \equiv T$
    \end{block}
